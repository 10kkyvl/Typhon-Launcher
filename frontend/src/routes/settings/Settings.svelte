<script lang="ts">
  ...
  let sendLogsConfirmOpen = $state(false);
...
{#if sendLogsConfirmOpen}
  <ConfirmModal prompt={sendLogsPrompt()} onconfirm={confirmSendLogs} onclose={() => (sendLogsConfirmOpen = false)} />
{/if}
