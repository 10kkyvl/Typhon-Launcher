...
describe('update reason localization', () => {
  it.each(['низкая уверенность в сопоставлении версий', 'размер раздачи сильно меньше установленной игры', 'unknown backend detail'])('does not render raw persisted reason %s in English', reason => {
    locale.set('en');
    const html = render(UpdateCard, { props: { update: update(reason), running: false } }).body;
    expect(html).not.toContain(reason);
    expect(html).toContain('Versions not comparable');
