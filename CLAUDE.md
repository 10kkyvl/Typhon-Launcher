...
Проектные скиллы лежат в `.claude/skills/<имя>/SKILL.md` и хранятся в git (в `.gitignore` для
них исключение `!.claude/skills/`). Вызываются как `/<имя>`, Claude Code подхватывает их сам.
Скилл — это повторяемая процедура с проверкой (релиз, съёмка окна, замер базиса), а не правило:
правила живут в этом файле. Новый скилл пишется по superpowers:writing-skills — сначала прогон
сабагента без скилла, потом со скиллом, и только затем он считается готовым.

- `release` — заметки к версии на двух языках, бамп версии, проверки, мерж и тег.
- `verify-live` — доказать правку в живом окне: сборка devmock, single instance, клики и скриншот на macOS, перезапуск на Windows, где логи и состояние.
- `lint-baseline` — замер и понижение базиса golangci-lint: что честно меряется с мака (darwin, windows), что только в CI (linux), чистое воркри.
- `bug-hunt` — поиск багов без известного симптома: охотники на Sonnet по скоупу и линзам, проверяющий с временным тестом, приёмка процитированных строк, отчёт без правок.
...
- Релиз собирает отдельный job на `macos-latest`, кладёт бандл в артефакты сборки, а
  ...
  прогонялся**: проверены только его части — сборка, упаковка, `signrelease`
  с двумя артефактами и приём такого манифеста клиентом.
