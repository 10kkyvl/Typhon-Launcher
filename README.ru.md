...
лаунчер по-прежнему качает, но поставить и запустить игру не может. Эта сборка пока не
подписана, поэтому при первом запуске Gatekeeper её не пропустит — откройте её один раз
через контекстное меню или снимите атрибут карантина руками. Версии под Linux нет.
