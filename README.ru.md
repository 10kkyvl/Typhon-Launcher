...
![Библиотека Typhon: игра, в которую можно продолжить играть, недавние игры и вся библиотека с обложками](.github/screenshots/library.webp)

| | |
|---|---|
| ![Установленные игры: сколько занято в хранилище библиотеки, метки обновлений на обложках](.github/screenshots/installed.webp) | ![Каталог всех игр с фильтрами по жанрам и поиском](.github/screenshots/catalog.webp) |
| ![Страница игры: описание, теги, сведения о платформах и доступное обновление](.github/screenshots/game.webp) | |
...
Основная сборка — под Windows. Сборка под macOS существует **только под Apple Silicon**
(интеловые маки не поддерживаются) и запускает игры через
[CrossOver](https://www.codeweavers.com/crossover), который ставится отдельно: без него
лаунчер по-прежнему качает, но поставить и запустить игру не может. Эта сборка не подписана
сертификатом Apple Developer и подписана не будет, поэтому скачанную копию Gatekeeper не
пропустит: разрешите её один раз в «Системных настройках» → «Конфиденциальность и
безопасность» либо снимите карантин командой
`xattr -dr com.apple.quarantine /Applications/Typhon.app`. Версии под Linux нет.
