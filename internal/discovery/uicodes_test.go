...
var (
	...
	tsCodePattern = regexp.MustCompile(`'(discovery\.[a-z0-9_]+)':`)
)
...
func TestErrorCodesMatchTheFrontendTable(t *testing.T) {
	goCodes := codesIn(t, goCodePattern, "service.go")
	if len(goCodes) < 3 {
		t.Fatalf("в пакете найдено %d кодов, ожидалось не меньше 3", len(goCodes))
	}
