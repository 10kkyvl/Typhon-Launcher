...
var (
	...
	tsCodePattern = regexp.MustCompile(`'(sources\.[a-z0-9_]+)':`)
)
...
func TestErrorCodesMatchTheFrontendTable(t *testing.T) {
	goCodes := codesIn(t, goCodePattern,
		"service.go", "releases.go",
		filepath.Join("feed", "feed.go"),
		filepath.Join("feed", "file.go"),
		filepath.Join("feed", "fetch.go"),
		filepath.Join("feed", "guard.go"),
	)
	if len(goCodes) < 20 {
		t.Fatalf("в пакете найдено %d кодов, ожидалось не меньше 20", len(goCodes))
	}

	tsPath := filepath.Join("..", "..", "frontend", "src", "lib", "sources", "sourceErrors.ts")
	...
	for _, code := range goCodes {
		if !inTS[code] {
			t.Errorf("код %q возвращается из Go, но не переводится в sourceErrors.ts", code)
		}
	}
	...
	for _, code := range tsCodes {
		if !inGo[code] {
			t.Errorf("код %q переводится в sourceErrors.ts, но Go его не возвращает", code)
		}
	}
}
