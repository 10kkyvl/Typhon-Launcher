...
var (
	...
	tsCodePattern = regexp.MustCompile(`'(settings\.[a-z0-9_]+)':`)
)
...
func TestErrorCodesMatchTheFrontendTable(t *testing.T) {
	...
	if len(goCodes) < 6 {
		t.Fatalf("в пакете найдено %d кодов, ожидалось не меньше 6", len(goCodes))
	}
	...
	for _, code := range tsCodes {
		if !inGo[code] {
			t.Errorf("код %q переводится в errLibrary.ts, но settings его не возвращает", code)
		}
	}
}
