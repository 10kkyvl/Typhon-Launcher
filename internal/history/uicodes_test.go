...
var (
	...
	tsCodePattern = regexp.MustCompile(`'(history\.[a-z0-9_]+)':`)
)
...
func TestErrorCodesMatchTheFrontendTable(t *testing.T) {
	...
	if len(goCodes) < 1 {
		t.Fatalf("в пакете найдено %d кодов, ожидался хотя бы 1", len(goCodes))
	}

	tsPath := filepath.Join("..", "..", "frontend", "src", "lib", "stores", "history.ts")
	...
	for _, code := range goCodes {
		if !inTS[code] {
			t.Errorf("код %q возвращается из Go, но не переводится в stores/history.ts", code)
		}
	}
	...
	for _, code := range tsCodes {
		if !inGo[code] {
			t.Errorf("код %q переводится в stores/history.ts, но history его не возвращает", code)
		}
	}
}
