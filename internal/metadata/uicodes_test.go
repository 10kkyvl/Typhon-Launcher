...
var (
	metadataGoCodePattern = regexp.MustCompile(`uierr\.(?:New|Wrap)\(\s*"([^"]+)"`)
	metadataTSCodePattern = regexp.MustCompile(`'(metadata\.[a-z0-9_]+)':`)
)

func metadataCodesIn(t *testing.T, pattern *regexp.Regexp, paths ...string) []string {
	t.Helper()
...
func TestErrorCodesMatchTheFrontendTable(t *testing.T) {
	...
	tsCodes := metadataCodesIn(t, metadataTSCodePattern, tsPath)
